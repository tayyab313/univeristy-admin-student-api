...
class staffController extends Controller
{
    ...
    public function getCourseStudentAttendance($courseId)
    {
        // Fetch the course with students and their attendance status
        $course = Course::with(['students' => function ($query) use ($courseId) {
            $query->wherePivot('is_approved', 1)
                ->with(['attendances' => function ($query) use ($courseId) {
        ...
        // Format the response to include students with their attendance status
        $students = $course->students->map(function ($student) {
            return [
                'id' => $student->id,
                'name' => $student->name,
                'attendance' => $student->attendance->first()->attendance ?? 'Not marked',
            ];
        });
